...
app.put('/update-user', authenticateUser, async (req, res) => {
...
app.delete("/api/properties/:id", authenticateUser, async (req, res) => {
  try {
...
app.put("/api/properties/:id", authenticateUser, upload.array("images", 5), async (req, res) => {
  try {
    ...
    const imageUrls = req.files.length > 0 ? req.files.map(file => file.path) : property.images;

    property.ListingType = form.ListingType;
    property.PropertyType = form.PropertyType;
    property.City = form.City;
    property.Area = parseFloat(form.Area);
    property.Bedrooms = parseInt(form['No. of Bedrooms'], 10);
    property.Latitude = parseFloat(form.Latitude);
    property.Longitude = parseFloat(form.Longitude);
    property.Price = parseFloat(form.Price);
    property.PredictedPrice = predictedPrice;
    property.BinaryFeatures = BinaryFeatures;
    property.images = imageUrls;
    property.Description = form.Description;
    property.Furnishing = form.Furnishing;
    property.Facing = form.Facing;
